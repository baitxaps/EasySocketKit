...
// positions
NSString * const TTVCSToastPositionTop             = @"top";
NSString * const TTVCSToastPositionCenter          = @"center";
NSString * const TTVCSToastPositionBottom          = @"bottom";
...
- (CGPoint)centerPointForPosition:(id)point withToast:(UIView *)toast {
    if([point isKindOfClass:[NSString class]]) {
        if([point caseInsensitiveCompare:TTVCSToastPositionTop] == NSOrderedSame) {
            return CGPointMake(self.bounds.size.width/2, (toast.frame.size.height / 2) + CSToastVerticalPadding);
        } else if([point caseInsensitiveCompare:TTVCSToastPositionCenter] == NSOrderedSame) {
...
//    }
    ...
    CGFloat wrapperWidth = MAX((imageWidth + (CSToastHorizontalPadding * 2)), (longerLeft + longerWidth + CSToastHorizontalPadding*2));
    ...
    if(messageLabel != nil) {
        ...
        messageLabel.center = CGPointMake(wrapperView.frame.size.width/2, wrapperView.frame.size.height/2);
